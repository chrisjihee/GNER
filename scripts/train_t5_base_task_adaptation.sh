...
deepspeed --include="localhost:0,1,2,3,4,5,6,7" --master_port $port src/run.py \
    ...
    --gradient_checkpointing True --bf16 True --tf32 True \
    --lr_scheduler_type constant --learning_rate 5e-05 --warmup_steps 0 \
    --num_train_epochs 12 \
    --run_name $RUN_NAME \
    ...
    --seed 1234 \
    --deepspeed $DEEPSPEED_CONFIG --max_train_samples 20480 --max_eval_samples 2048 --max_predict_samples 2048
